...
# ======================================================================
# [PUBLICAÇÃO / SEGURANÇA] FLASK + VARIÁVEIS DE AMBIENTE
# ======================================================================
app = Flask(__name__)

BASE_DIR = Path(__file__).resolve().parent

# DATA_DIR permite colocar banco, uploads e anexos em um disco persistente.
# Sem DATA_DIR, a V20 continua usando a própria pasta do projeto.
STORAGE_DIR = Path(os.environ.get("DATA_DIR", str(BASE_DIR))).expanduser().resolve()
STORAGE_DIR.mkdir(parents=True, exist_ok=True)

# Em produção configure SECRET_KEY no Render.
...
# Banco SQLite. Com Persistent Disk no Render, use DATA_DIR=/var/data.
...
def iniciar_banco():
    with conectar_banco() as conexao:
        ...
        if not existe:
            # ==========================================================
            # [PUBLICAÇÃO / SEGURANÇA] ADMIN INICIAL POR ENVIRONMENT
            # ==========================================================
            ...
            if admin_email and admin_password:
                ...
                print("[!] Nenhum administrador cadastrado. Configure ADMIN_EMAIL e ADMIN_PASSWORD.")
...
if __name__ == "__main__":
    ...
    # No IDLE continua em 127.0.0.1:5000 por padrão.
    # No Render o Gunicorn importa "app", então este bloco não é executado.
    IP_LOCAL = os.environ.get("HOST", "127.0.0.1")
    PORTA_ACESSO = int(os.environ.get("PORT", "5000"))
    DEBUG_LOCAL = os.environ.get("FLASK_DEBUG", "0").strip() == "1"

    if os.environ.get("ABRIR_NAVEGADOR", "1").strip() == "1" and IP_LOCAL in {"127.0.0.1", "localhost"}:
