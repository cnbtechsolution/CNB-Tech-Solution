# ======================================================================
# CNB TECH SOLUTION - V20
# Versão preparada para IDLE + GitHub + Render
# Login administrativo + botão Esqueci minha senha + recuperação por chave preservados.
# Segredos de produção ficam em variáveis de ambiente (não no GitHub).
# ======================================================================
...
# ----------------------------------------------------------------------
# -------- [V20: FLASK + SEGURANÇA PARA LOCAL / RENDER] ----------------
# ----------------------------------------------------------------------
# Flask = servidor web principal do projeto.
app = Flask(__name__)

# No Render, configure SECRET_KEY em Environment Variables.
# No IDLE/local, existe somente uma chave de desenvolvimento para facilitar testes.
AMBIENTE_RENDER = bool(os.environ.get("RENDER"))
app.secret_key = os.environ.get("SECRET_KEY", "cnb-tech-v20-chave-somente-local")
app.config["SESSION_COOKIE_HTTPONLY"] = True
app.config["SESSION_COOKIE_SAMESITE"] = "Lax"
if AMBIENTE_RENDER:
    app.config["SESSION_COOKIE_SECURE"] = True

# Rota simples para o Render verificar se o serviço está no ar sem gerar visita.
@app.route("/health")
def health():
    return {"status": "ok", "app": "CNB Tech Solution", "versao": "V20"}, 200

# Banco SQLite. Em produção pode apontar para um diretório persistente usando
# CNB_DATA_DIR (ex.: /var/data quando houver Persistent Disk no Render).
BASE_DIR = Path(__file__).resolve().parent
DATA_DIR = Path(os.environ.get("CNB_DATA_DIR", str(BASE_DIR))).resolve()
DATA_DIR.mkdir(parents=True, exist_ok=True)
BANCO_DADOS = DATA_DIR / "cnb_tech_solution.db"
...
PASTA_MIDIA_SITE = DATA_DIR / "midia_site"
PASTA_MIDIA_SITE.mkdir(exist_ok=True)
EXTENSOES_LOGO_SITE = {"png", "webp", "jpg", "jpeg"}
EXTENSOES_VIDEO_SITE = {"mp4"}

# Permite upload do vídeo da Home pelo painel até 250 MB.
# Os anexos de orçamento continuam limitados individualmente a 24 MB.
app.config["MAX_CONTENT_LENGTH"] = 250 * 1024 * 1024  # <<<< [LIMITE GLOBAL DE UPLOAD: 250 MB]

# ----------------------------------------------------------------------
# -------- [ARQUIVOS DOS ORÇAMENTOS: PDF / DOCX / POWERPOINT] ----------
# ----------------------------------------------------------------------
PASTA_ANEXOS = DATA_DIR / "anexos_orcamentos"
PASTA_ANEXOS.mkdir(exist_ok=True)
EXTENSOES_PERMITIDAS = {"pdf", "docx", "ppt", "pptx"}
LIMITE_ANEXO_ORCAMENTO = 24 * 1024 * 1024  # <<<< [LIMITE DOS DOCUMENTOS: 24 MB]

def arquivo_permitido(nome_arquivo):
    return "." in nome_arquivo and nome_arquivo.rsplit(".", 1)[1].lower() in EXTENSOES_PERMITIDAS

def arquivo_dentro_limite(arquivo, limite=LIMITE_ANEXO_ORCAMENTO):
    # Mede o arquivo sem consumi-lo; depois volta o cursor ao início.
...
def iniciar_banco():
    with conectar_banco() as conexao:
        ...
        # [V20 - ADMIN SEGURO]
        # Não existe mais senha padrão escrita no código.
        # Se ADMIN_EMAIL + ADMIN_INITIAL_PASSWORD forem informados, a primeira conta
        # pode ser criada automaticamente. Caso contrário, use /admin/redefinir-senha
        # com RESET_ADMIN_TOKEN para criar/atualizar o administrador master.
        existe = conexao.execute("SELECT id FROM administradores LIMIT 1").fetchone()
        if not existe:
            admin_email_inicial = os.environ.get("ADMIN_EMAIL", "").strip().lower()
            admin_senha_inicial = os.environ.get("ADMIN_INITIAL_PASSWORD", "")
            admin_nome_inicial = os.environ.get("ADMIN_NAME", "Administrador Master").strip() or "Administrador Master"

            if admin_email_inicial and admin_senha_inicial:
                conexao.execute(
                    "INSERT INTO administradores (nome,email,senha_hash,perfil,ativo,criado_em) VALUES (?,?,?,?,?,?)",
                    (admin_nome_inicial, admin_email_inicial, generate_password_hash(admin_senha_inicial), "master", 1, datetime.now().strftime("%d/%m/%Y %H:%M"))
                )
...
def solicitar_orcamento(slug):
    ...
    if request.method == 'POST':
        ...
        if arquivo and arquivo.filename and not arquivo_dentro_limite(arquivo):
            return "O arquivo ultrapassa o limite de 24 MB.", 413
...
def admin_login():
    ...
    return render_template_string("""<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Login CNB Tech</title>{{css|safe}}</head><body><main class='wrap'><div class='form-card' style='margin:80px auto;max-width:520px'><h1>⚡ CNB TECH SOLUTION</h1><p class='muted'>Acesso administrativo</p>{% if erro %}<div class='alerta'>{{erro}}</div>{% endif %}<form method='post'><label>E-mail</label><input class='campo' type='email' name='email' required><label>Senha</label><input class='campo' type='password' name='senha' required><button class='btn'>Entrar</button></form><div style='margin-top:14px;text-align:center'><a class='btn btn-sec' href='/admin/esqueci-senha' style='display:inline-block;text-decoration:none'>Esqueci minha senha</a></div><p class='muted' style='margin-top:18px'>Use o e-mail administrativo e a chave de recuperação configurados no Render.</p></div></main></body></html>""",css=ADMIN_BASE_CSS,erro=erro)


# ----------------------------------------------------------------------
# -------- [V20 / BOTÃO "ESQUECI MINHA SENHA" DO LOGIN] ---------------
# ----------------------------------------------------------------------
# Esta tela usa RESET_ADMIN_TOKEN configurado no Render. A chave não é
# gravada no GitHub e não aparece na página. O administrador informa a
# chave para seguir até a tela segura de redefinição de senha.
        ...
        <body>
            <main class="wrap">
                <div class="form-card" style="margin:80px auto;max-width:520px">
                    ...
                    <p class="muted">Informe a chave de recuperação configurada no Render.</p>
...
def admin_diagnostico():
    ...
    dados = {
        ...
        "aplicacao": "CNB Tech Solution V20",
...
def anexar_arquivo_orcamento(orcamento_id):
    ...
    if not arquivo_permitido(arquivo.filename):
        ...
        return redirect(url_for('admin_orcamentos'))
    if not arquivo_dentro_limite(arquivo):
        flash('Arquivo muito grande. O limite para documentos é 24 MB.')
        return redirect(url_for('admin_orcamentos'))
...
# ----------------------------------------------------------------------
# ----------- [V20: INICIALIZAÇÃO COMPATÍVEL COM GUNICORN] -------------
# ----------------------------------------------------------------------
# O Render usa `gunicorn app:app`. Nesse modo o bloco __main__ não roda,
# então o banco precisa ser preparado durante a importação do aplicativo.
iniciar_banco()
...
if __name__ == "__main__":
    IP_LOCAL = "0.0.0.0" if AMBIENTE_RENDER else "127.0.0.1"
    PORTA_ACESSO = int(os.environ.get("PORT", "5000"))

    # No IDLE abre o navegador automaticamente. No Render isso fica desativado.
    if not AMBIENTE_RENDER:
        Timer(1.5, abrir_navegador, args=[PORTA_ACESSO]).start()

    print(f"\n[+] CNB Tech Solution V20 iniciada com sucesso!")
    ...
    # Inicia o Flask
    app.run(host=IP_LOCAL, port=PORTA_ACESSO, debug=not AMBIENTE_RENDER, use_reloader=False)
