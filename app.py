...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "troque-esta-chave-secreta-cnb-tech-local")
...
def iniciar_banco():
    with conectar_banco() as conexao:
        ...
        if not existe:
            # [PUBLICAÇÃO / SEGURANÇA] Cria o primeiro admin somente pelas variáveis do Render.
            ...
            if admin_email and admin_password:
                conexao.execute(
                    ...
                    (admin_name, admin_email, generate_password_hash(admin_password), "master", 1, datetime.now().strftime("%d/%m/%Y %H:%M"))
                )

# [RENDER / GUNICORN] Inicializa as tabelas também quando o app é importado pelo Gunicorn.
# CREATE TABLE IF NOT EXISTS preserva os dados existentes.
iniciar_banco()
